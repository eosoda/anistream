...
Somente `anistream` recebe o domínio `https://beta.<dominio>` e a porta
...
1. Aponte `beta.<dominio>` para o IP da VPS.
...
6. Execute `npx prisma migrate deploy` como comando de deploy do AniStream. O entrypoint também executa a migration de forma idempotente antes de iniciar o servidor.
...
9. Adicione Basic Auth temporário no domínio ou uma allowlist equivalente no proxy/controle de acesso do beta.
...
Se um volume antigo já tiver tabelas criadas por `prisma db push` mas não tiver
