  ...
  return (
    <div className="max-w-7xl mx-auto px-4 md:px-8 py-8 space-y-8">
      <div className="max-w-2xl mx-auto text-center space-y-4">
        <h1 className="text-2xl md:text-4xl font-black text-white tracking-tight">Pesquisa de Animes</h1>
      ...
      {searchData && searchData.pagination.totalPages > 1 && (
        <div className="flex items-center justify-center gap-4 pt-8">
          <button onClick={() => setPage((value) => Math.max(value - 1, 1))} disabled={!searchData.pagination.hasPreviousPage} className="flex items-center gap-2 px-5 py-2.5 rounded-full font-bold text-xs bg-white/10 enabled:hover:bg-[#FF6B00] text-white disabled:opacity-40 disabled:cursor-not-allowed">
            <ChevronLeft size={16} /> Anterior
          </button>
          <span className="text-xs font-bold text-gray-300 px-3 py-1.5 rounded-lg bg-white/5">Página {searchData.pagination.currentPage} de {searchData.pagination.totalPages}</span>
          <button onClick={() => setPage((value) => value + 1)} disabled={!searchData.pagination.hasNextPage} className="flex items-center gap-2 px-5 py-2.5 rounded-full font-bold text-xs bg-white/10 enabled:hover:bg-[#FF6B00] text-white disabled:opacity-40 disabled:cursor-not-allowed">
...
export default function SearchPage() {
  return <Suspense fallback={<div className="p-12 text-center text-gray-400">Carregando pesquisa...</div>}><SearchContent /></Suspense>;
}
