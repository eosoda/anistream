  ...
  return <div className="space-y-5 pb-28">
    ...
    <div id={`navigation-panel-${activeTab}`} role="tabpanel" aria-labelledby={`navigation-tab-${activeTab}`}>
      ...
      {activeTab === 'mobile' && <AdminPanel>
        ...
        <div className="grid gap-4 lg:grid-cols-3">{state.mobileBottomIds.map((id, index) => { const current = state.navigation.find((item) => item.id === id); const options = state.navigation.filter((item) => item.enabled && (item.id === id || (preview.desktop.some((visible) => visible.id === item.id) && !state.mobileBottomIds.includes(item.id)))); return <label key={`mobile-slot-${index}`} className="admin-field-group"><span className="admin-field-label">Slot {index + 1}</span><select className="admin-field" value={id} onChange={(event) => setMobileSlot(index, event.target.value as NavDestinationId)} aria-invalid={hasIssue(issues, 'mobileBottomIds', index)}>{options.map((item) => <option key={item.id} value={item.id}>{item.label} · {item.href}</option>)}</select>{current && <span className="text-[11px] text-[var(--admin-dim)]">Publicado como {current.label}</span>}</label>; })}</div>
        <div className="mt-5 border-t border-[var(--admin-line)] pt-5"><p className="admin-eyebrow">Preview</p><div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-4">{preview.mobileBottom.map((item, index) => <PreviewItem key={`${item.id}-${index}`} item={item} />)}</div></div>
      </AdminPanel>}
