...
const BLOCK_OPTIONS: Array<{
  ...
  {
    type: 'hero',
    label: 'Hero de destaques',
    description: 'Destaque Kenjitsu em até cinco slides.',
  },
  {
    type: 'catalog_carousel',
    label: 'Carrossel de catálogo',
    description: 'Consulta ou coleção manual de animes.',
  },
  {
    type: 'continue_watching',
    label: 'Continuar assistindo',
    description: 'Progresso salvo neste navegador.',
  },
  {
    type: 'quick_filters',
    label: 'Filtros rápidos',
    description: 'Atalhos para a pesquisa do catálogo.',
  },
  {
    type: 'editorial_notice',
    label: 'Aviso editorial',
    description: 'Texto simples com CTA interno.',
  },
  {
    type: 'divider',
    label: 'Separador',
    description: 'Divisão visual entre grupos.',
  },
];

const DEFAULT_FRAME: HomepageFrame = {
  width: 'content',
  variant: 'default',
  spacing: 'normal',
};
const QUERY_SOURCE_DEFAULT: HomepageQuerySource = {
  mode: 'query',
  source: 'top',
  category: 'popular',
};
...
function createBlock(type: HomepageBlock['type'], order: number): HomepageBlock {
  ...
  if (type === 'hero')
    return {
      ...common,
      type,
      frame: { ...DEFAULT_FRAME, width: 'full', variant: 'featured' },
      source: { ...QUERY_SOURCE_DEFAULT, category: 'trending' },
      slideLimit: 5,
      autoplay: 'standard',
    };
  if (type === 'catalog_carousel')
    return {
      ...common,
      type,
      source: QUERY_SOURCE_DEFAULT,
      title: 'Nova seção',
      subtitle: 'Conteúdo selecionado pelo Kenjitsu.',
      limit: 8,
      layout: 'carousel',
      preCache: false,
      ctaHref: '/populares',
  ...
  if (type === 'quick_filters')
    return {
      ...common,
      type,
      title: 'Explore por filtro',
      frame: { ...DEFAULT_FRAME, spacing: 'compact' },
    };
  if (type === 'editorial_notice')
    return {
      ...common,
      type,
      title: 'Aviso',
      body: 'Escreva uma mensagem para quem visita a Home.',
      variant: 'info',
      active: true,
    };
...
function blockDescription(block: HomepageBlock) {
  if (block.type === 'hero' || block.type === 'catalog_carousel') {
    if (block.source.mode === 'manual') return 'IDs manuais do Kenjitsu';
    if (block.source.mode === 'collection') return `Coleção editorial: ${block.source.slug}`;
    return `Consulta ${block.source.source} do Kenjitsu`;
  }
...
function SortableHomepageBlock({
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`group flex items-stretch gap-2 border bg-[var(--admin-surface)] p-2 transition-colors ${selected ? 'border-[#FF6B00]' : 'border-[var(--admin-line)]'} ${isDragging ? 'z-10 opacity-70 shadow-2xl' : ''}`}
    >
      <button
        type="button"
        className="grid min-h-16 w-10 shrink-0 place-items-center border border-[var(--admin-line)] text-[var(--admin-dim)] hover:text-[var(--admin-text)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#FF6B00]"
        aria-label={`Arrastar ${blockLabel(block)}`}
        {...attributes}
        {...listeners}
      >
      ...
      <button
        type="button"
        onClick={onSelect}
        className="min-w-0 flex-1 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#FF6B00] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--admin-surface)]"
      >
        <span className="flex items-center gap-2 font-mono text-[10px] uppercase tracking-widest text-[var(--admin-dim)]">
          <span>{String(block.order).padStart(2, '0')}</span>
      ...
      <div className="flex shrink-0 flex-col items-center justify-center gap-1 sm:flex-row">
        <button
          ...
          aria-label={block.enabled ? `Ocultar ${blockLabel(block)}` : `Exibir ${blockLabel(block)}`}
          title={block.enabled ? 'Ocultar' : 'Exibir'}
        >
          {block.enabled ? <Eye size={15} /> : <EyeOff size={15} />}
        </button>
        <button type="button" onClick={onDuplicate} className="admin-icon-button" aria-label={`Duplicar ${blockLabel(block)}`} title="Duplicar">
          <Copy size={15} />
        </button>
        <button
          type="button"
          onClick={onRemove}
          className="admin-icon-button text-rose-300 hover:border-rose-400/50 hover:text-rose-200"
...
function Field({ label, children, hint }: { label: string; children: ReactNode; hint?: string }) {
  return (
    <label className="admin-field-group">
      ...
      {children}
      {hint && <span className="text-[11px] text-[var(--admin-dim)]">{hint}</span>}
    </label>
  );
}
    ...
    <div className="space-y-3 border-t border-[var(--admin-line)] pt-4">
      <Field label="Modo de fonte">
          ...
          onChange={(event) =>
            onChange(
              event.target.value === 'manual'
                ? { mode: 'manual', anilistIds: ['52991'] }
                : event.target.value === 'collection'
                  ? { mode: 'collection', slug: 'destaques-da-semana' }
        ...
        >
          ...
          <option value="collection">Coleção editorial</option>
        </select>
      </Field>
      ...
      {source.mode === 'manual' ? (
        <Field label="anilistIds" hint="Um ID por linha ou separado por vírgula. Dados resolvidos sempre pelo Kenjitsu.">
          <textarea
            className="admin-field min-h-28 resize-y font-mono text-xs"
            value={source.anilistIds.join('\n')}
            onChange={(event) =>
              onChange({
                mode: 'manual',
        ...
        <Field label="Slug da coleção" hint="Crie e gerencie coleções em Personalização → Coleções.">
          <input
        ...
        <>
          <Field label="Tipo de consulta">
            <select
              ...
              onChange={(event) =>
                updateQuery({
                  source: event.target.value as HomepageQuerySource['source'],
                  category: event.target.value === 'top' ? 'popular' : undefined,
                  year: event.target.value === 'season' ? new Date().getFullYear() : undefined,
                  season: event.target.value === 'season' ? 'summer' : undefined,
                  filters: event.target.value === 'catalog' ? {} : undefined,
                })
              }
            >
          ...
          {source.source === 'top' && (
            <Field label="Categoria">
              <select
                className="admin-field"
                value={source.category || 'popular'}
                onChange={(event) =>
                  updateQuery({
                    category: event.target.value as HomepageQuerySource['category'],
                  })
                }
              >
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field label="Ano">
                <input
                  ...
                  min={1900}
                  max={2200}
                  value={source.year || new Date().getFullYear()}
                  onChange={(event) => updateQuery({ year: Number(event.target.value) })}
                />
              </Field>
              <Field label="Estação">
                <select
                  className="admin-field"
                  value={source.season || 'summer'}
                  onChange={(event) =>
                    updateQuery({
                      season: event.target.value as HomepageQuerySource['season'],
                    })
                  }
                >
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <Field label="Gêneros">
                  <input
                    ...
                    onChange={(event) =>
                      updateQuery({
                        filters: { ...filters, genres: event.target.value },
                      })
                    }
                    placeholder="1,10,22"
                  />
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      updateQuery({
                        filters: {
                          ...filters,
                          orderBy: event.target.value as typeof filters.orderBy,
                        },
                      })
                    }
                  >
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3">
        <Field label="Visibilidade">
          <select
            className="admin-field"
            value={block.enabled ? 'enabled' : 'hidden'}
            onChange={(event) => update({ enabled: event.target.value === 'enabled' })}
          >
            <option value="enabled">Visível</option>
        ...
        <Field label="Largura">
          <select
            className="admin-field"
            value={block.frame.width}
            onChange={(event) =>
              updateFrame({
                width: event.target.value as HomepageFrame['width'],
              })
            }
          >
      ...
      <div className="grid grid-cols-2 gap-3">
        <Field label="Variante">
          <select
            className="admin-field"
            value={block.frame.variant}
            onChange={(event) =>
              updateFrame({
                variant: event.target.value as HomepageFrame['variant'],
              })
            }
          >
        ...
        <Field label="Espaçamento">
          <select
            className="admin-field"
            value={block.frame.spacing}
            onChange={(event) =>
              updateFrame({
                spacing: event.target.value as HomepageFrame['spacing'],
              })
            }
          >
      ...
      <div className="grid gap-3 border-t border-[var(--admin-line)] pt-4 sm:grid-cols-2">
        <Field label="Fundo da seção" hint="Somente cor hexadecimal; CSS arbitrário não é aceito.">
          <div className="flex gap-2">
            <input
              type="color"
              className="h-11 w-12 shrink-0 cursor-pointer border border-[var(--admin-line)] bg-transparent p-1"
              value={block.background || '#0b0b0f'}
              onChange={(event) => update({ background: event.target.value })}
            />
            <input
              className="admin-field font-mono text-xs"
              value={block.background || ''}
              onChange={(event) => update({ background: event.target.value || undefined })}
              placeholder="#0b0b0f"
            />
          </div>
        </Field>
        <div className="grid grid-cols-2 gap-2">
          <label className="flex min-h-11 cursor-pointer items-center gap-2 border border-[var(--admin-line)] bg-[var(--admin-panel-raised)] p-3 text-xs font-semibold text-[var(--admin-text)]">
            <input
              type="checkbox"
              ...
              onChange={(event) =>
                update({
                  visibility: {
                    ...block.visibility,
                    desktop: event.currentTarget.checked,
                  },
                })
              }
            />
            ...
            <input
              ...
              onChange={(event) =>
                update({
                  visibility: {
                    ...block.visibility,
                    mobile: event.currentTarget.checked,
                  },
                })
              }
            />
        ...
        <div className="space-y-3 border-t border-[var(--admin-line)] pt-4">
          <div className="grid grid-cols-2 gap-3">
            <Field label="Slides">
              <input
                ...
                min={1}
                max={5}
                value={block.slideLimit}
                onChange={(event) =>
                  update({
                    slideLimit: Math.min(5, Math.max(1, Number(event.target.value) || 1)),
                  })
                }
              />
            </Field>
            <Field label="Autoplay">
              <select
                ...
                onChange={(event) =>
                  update({
                    autoplay: event.target.value as typeof block.autoplay,
                  })
                }
              >
          ...
          <Field label="Título opcional">
            <input
              ...
              onChange={(event) => update({ titleOverride: event.target.value || undefined })}
              placeholder="Usar título do Kenjitsu"
            />
          </Field>
          <Field label="Texto opcional">
            <textarea
              className="admin-field min-h-20 resize-y"
              value={block.subtitleOverride || ''}
              onChange={(event) => update({ subtitleOverride: event.target.value || undefined })}
              placeholder="Usar sinopse do Kenjitsu"
            />
          </Field>
        </div>
      )}
      ...
      {block.type === 'catalog_carousel' && (
        <div className="space-y-3 border-t border-[var(--admin-line)] pt-4">
          <Field label="Título">
            <input className="admin-field" value={block.title} onChange={(event) => update({ title: event.target.value })} />
          </Field>
          <Field label="Subtítulo">
            <input className="admin-field" value={block.subtitle || ''} onChange={(event) => update({ subtitle: event.target.value || undefined })} />
          </Field>
          <div className="grid grid-cols-2 gap-3">
            <Field label="Itens">
              <input
                ...
                onChange={(event) =>
                  update({
                    limit: Math.min(12, Math.max(6, Number(event.target.value) || 8)),
                  })
                }
              />
              <span className="text-[11px] text-[var(--admin-dim)]">Entre 6 e 12.</span>
            </Field>
            <Field label="Layout">
              <select className="admin-field" value={block.layout} onChange={(event) => update({ layout: event.target.value as typeof block.layout })}>
          ...
          <Field label="Mensagem vazia">
            <input
              className="admin-field"
              value={block.emptyState || ''}
              onChange={(event) => update({ emptyState: event.target.value || undefined })}
              placeholder="Nenhum título disponível."
            />
          </Field>
          <label className="flex min-h-11 cursor-pointer items-center gap-2 border border-[var(--admin-line)] bg-[var(--admin-panel-raised)] p-3 text-xs font-semibold text-[var(--admin-text)]">
            <input
              type="checkbox"
              className="size-4 accent-[var(--accent)]"
              checked={block.preCache}
              onChange={(event) => update({ preCache: event.currentTarget.checked })}
            />
            Pré-cachear esta seção
          </label>
          <div className="grid grid-cols-2 gap-3">
            <Field label="CTA">
              <input
                ...
                onChange={(event) => update({ ctaLabel: event.target.value || undefined })}
                placeholder="Ver todos"
              />
            </Field>
            <Field label="Rota interna do CTA">
              <input
                className="admin-field font-mono text-xs"
                value={block.ctaHref || ''}
                onChange={(event) => update({ ctaHref: event.target.value || undefined })}
      ...
      {block.type === 'continue_watching' && (
        <Field label="Título">
          <input className="admin-field" value={block.title || ''} onChange={(event) => update({ title: event.target.value || undefined })} />
        </Field>
      )}
      ...
      {block.type === 'editorial_notice' && (
        <div className="space-y-3 border-t border-[var(--admin-line)] pt-4">
          <Field label="Status">
            <select
              className="admin-field"
              value={block.active ? 'active' : 'inactive'}
          ...
          <Field label="Texto simples">
            <textarea className="admin-field min-h-28 resize-y" value={block.body} onChange={(event) => update({ body: event.target.value })} />
          </Field>
          <Field label="Variante">
            <select className="admin-field" value={block.variant} onChange={(event) => update({ variant: event.target.value as typeof block.variant })}>
      ...
      {block.type === 'divider' && (
        <Field label="Rótulo opcional">
          <input className="admin-field" value={block.label || ''} onChange={(event) => update({ label: event.target.value || undefined })} />
        </Field>
      )}
    </div>
  );
...
export default function AdminHomepagePage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setFeedback({
        tone: 'danger',
        message: error instanceof Error ? error.message : 'Não foi possível carregar o builder.',
      });
    } finally {
      setLoading(false);
    }
  ...
  const moveBlock = (id: string, direction: -1 | 1) => {
    ...
    updateDocument((document) => ({
      ...document,
      blocks: arrayMove(orderedBlocks, index, target).map((block, itemIndex) => ({ ...block, order: itemIndex + 1 })),
    }));
    setSelectedId(id);
  };
  ...
  const removeBlock = async (block: HomepageBlock) => {
    ...
    const confirmed = await confirm({
      title: 'Remover bloco?',
      description: `O bloco “${blockLabel(block)}” será removido do rascunho. A Home publicada não muda até publicar.`,
      confirmText: 'Remover',
      cancelText: 'Cancelar',
      variant: 'danger',
    });
    ...
    updateDocument((document) => ({
      ...document,
      blocks: document.blocks.filter((item) => item.id !== block.id).map((item, index) => ({ ...item, order: index + 1 })),
    }));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const payload = await response.json();
      if (!response.ok)
        throw Object.assign(new Error(payload.error?.message || 'Não foi possível salvar o rascunho.'), { code: payload.error?.code, status: response.status });
      setState(payload.data);
      setSavedDraft(JSON.stringify(payload.data.draft));
      setFeedback({
        tone: 'success',
        message: 'Rascunho salvo. A Home pública continua igual até a publicação.',
      });
    } catch (error) {
  ...
  const handlePublish = async () => {
    ...
    const confirmed = await confirm({
      title: 'Publicar nova Home?',
      description: `${state.summary.visibleBlockCount} bloco(s) visível(is) serão publicados para todos os visitantes.`,
      confirmText: 'Publicar Home',
      cancelText: 'Cancelar',
      variant: 'primary',
    });
    if (!confirmed) return;
    ...
    try {
      ...
      setFeedback({
        tone: 'danger',
        message: error instanceof Error ? error.message : 'Não foi possível publicar a Home.',
      });
    } finally {
      setSaving(false);
    }
  ...
  const handleDiscardServerDraft = async () => {
    if (!state || saving) return;
    const confirmed = await confirm({
      title: 'Restaurar última publicação?',
      description: 'Todas as alterações salvas no rascunho serão substituídas pela última Home publicada.',
    ...
    setSaving(true);
    try {
      const response = await fetch('/api/admin/homepage/discard', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          expectedDraftVersion: state.draftVersion,
      ...
      setFeedback({
        tone: 'success',
        message: 'Rascunho restaurado para a última publicação.',
      });
    } catch (error) {
      setFeedback({
        tone: 'danger',
        message: error instanceof Error ? error.message : 'Não foi possível restaurar o rascunho.',
      });
    } finally {
      setSaving(false);
    }
  ...
  const openSnapshot = async (snapshot: HomepageSnapshotSummary) => {
    ...
    try {
      ...
      setFeedback({
        tone: 'danger',
        message: error instanceof Error ? error.message : 'Não foi possível carregar o snapshot.',
      });
    } finally {
  ...
  const createSnapshot = async () => {
    ...
    try {
      ...
      setFeedback({
        tone: 'success',
        message: `${payload.data.snapshot?.label || 'Snapshot'} disponível no histórico.`,
      });
    } catch (error) {
      const status = error && typeof error === 'object' && 'status' in error ? error.status : 0;
      setFeedback({
  ...
  const restoreSnapshot = async (snapshot: HomepageSnapshotSummary) => {
    ...
    try {
      ...
      setFeedback({
        tone: 'success',
        message: `${snapshot.label} restaurado no rascunho. Salve para continuar.`,
      });
    } catch (error) {
      const status = error && typeof error === 'object' && 'status' in error ? error.status : 0;
      setFeedback({
  ...
  const deleteSnapshot = async (snapshot: HomepageSnapshotSummary) => {
    ...
    if (isCurrentPublished) {
      setFeedback({
        tone: 'warning',
        message: 'A publicação atual da Home é protegida e não pode ser excluída.',
      });
      return;
    }
    ...
    try {
      ...
      setState((current) =>
        current
          ? {
              ...current,
              snapshots: current.snapshots.filter((item) => item.id !== deleted.id),
            }
          : current,
      );
      ...
      setFeedback({
        tone: 'success',
        message: `${deleted.label} foi excluído do histórico.`,
      });
    } catch (error) {
      const status = error && typeof error === 'object' && 'status' in error ? error.status : 0;
      setFeedback({
        tone: status === 409 ? 'warning' : 'danger',
        message: error instanceof Error ? error.message : 'Não foi possível excluir o snapshot.',
      });
    } finally {
  ...
  if (loading)
    return (
      <div className="admin-empty-state min-h-[420px]" aria-live="polite">
        ...
        <h2>Carregando construtor da Home</h2>
        <p>Consultando o layout publicado e o rascunho administrativo.</p>
      </div>
    );
  if (!state)
    return (
      <div className="admin-empty-state min-h-[420px]">
        <h2>Builder indisponível</h2>
        <p>Não foi possível carregar a configuração da Home.</p>
  ...
  return (
    <div className="space-y-5 pb-28">
      <AdminPageHeader
        ...
        description="Componha a página inicial com blocos Kenjitsu, conteúdo editorial seguro e prévia responsiva."
        breadcrumbs={[{ label: 'Navegação', href: '/admin/navigation' }, { label: 'Home' }]}
        status={<AdminStatusBadge status="healthy" label={`Publicada v${state.publishedVersion}`} />}
        actions={
          <div className="flex flex-wrap gap-2">
            <Link href="/admin/navigation" className="admin-button is-ghost">
              Navegação
            </Link>
            <button type="button" className="admin-button is-ghost" onClick={() => window.open('/preview/homepage', '_blank', 'noopener,noreferrer')}>
              <ExternalLink size={15} /> Abrir prévia
            </button>
            <button type="button" className="admin-button is-primary" onClick={() => void handlePublish()} disabled={saving || dirty}>
              {saving ? <Loader2 size={15} className="animate-spin" /> : <MonitorPlay size={15} />} Publicar
            </button>
          </div>
        }
      />
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1.1fr)_minmax(360px,0.9fr)]">
        <div className="space-y-5">
          <AdminPanel>
            <div className="admin-panel-header">
              <div>
                <p className="admin-eyebrow">Canvas operacional</p>
                <h2 className="admin-section-title">Blocos da Home</h2>
                <p className="admin-section-description">Arraste para reordenar ou use o teclado. A composição aceita até 12 blocos.</p>
              </div>
              <div className="flex items-center gap-2">
                <span className="font-mono text-[11px] text-[var(--admin-dim)]">{orderedBlocks.length}/12</span>
                <button
                  type="button"
                  className="admin-button is-primary"
                  onClick={() => setShowAddBlock((value) => !value)}
                  disabled={orderedBlocks.length >= 12}
                >
            ...
            {showAddBlock && (
              <div className="mx-4 mb-4 grid gap-2 border border-[#FF6B00]/30 bg-[#FF6B00]/[0.05] p-3 sm:mx-5 sm:grid-cols-2">
                <p className="sm:col-span-2 text-xs font-bold uppercase tracking-wider text-[#FFB27A]">Escolha um bloco tipado</p>
                {BLOCK_OPTIONS.map((option) => (
                  <button
                    key={option.type}
                    type="button"
                    onClick={() => addBlock(option.type)}
                    className="flex items-start gap-3 border border-[var(--admin-line)] bg-[var(--admin-surface)] p-3 text-left transition-colors hover:border-[#FF6B00]/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#FF6B00]"
                  >
                    <LayoutTemplate size={16} className="mt-0.5 shrink-0 text-[#FF6B00]" />
                    <span>
                      <strong className="block text-xs text-[var(--admin-text)]">{option.label}</strong>
                      <small className="mt-1 block text-[11px] text-[var(--admin-dim)]">{option.description}</small>
                    </span>
                  </button>
                ))}
              </div>
            ...
            <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
              <SortableContext items={orderedBlocks.map((block) => block.id)} strategy={verticalListSortingStrategy}>
                <div className="space-y-2" aria-label="Blocos ordenáveis da Home">
            ...
            <div className="mx-4 mb-4 mt-4 flex flex-wrap items-center gap-2 border-t border-[var(--admin-line)] pt-4 text-xs text-[var(--admin-dim)] sm:mx-5 sm:mb-5">
              <AlignJustify size={14} />
              <span>Bloco selecionado: {selectedBlock ? blockLabel(selectedBlock) : 'nenhum'}</span>
              {selectedBlock && (
                <>
                  <button type="button" className="admin-icon-button" onClick={() => moveBlock(selectedBlock.id, -1)} aria-label="Mover bloco para cima">
                    <ArrowUp size={14} />
                  </button>
                  <button type="button" className="admin-icon-button" onClick={() => moveBlock(selectedBlock.id, 1)} aria-label="Mover bloco para baixo">
                    <ArrowDown size={14} />
                  </button>
                </>
              )}
        ...
        <AdminPanel className="h-fit xl:sticky xl:top-5">
          <div className="admin-panel-header">
            <div>
              <p className="admin-eyebrow">Inspector</p>
              <h2 className="admin-section-title">{selectedBlock ? blockLabel(selectedBlock) : 'Selecione um bloco'}</h2>
              <p className="admin-section-description">Configurações tipadas e seguras para o bloco selecionado.</p>
            </div>
          </div>
          {selectedBlock ? (
            <div className="p-4 sm:p-5">
              <BlockInspector block={selectedBlock} onChange={(updater) => updateBlock(selectedBlock.id, updater)} />
            </div>
          ) : (
            <div className="admin-empty-state">
              <h2>Nenhum bloco selecionado</h2>
              <p>Escolha um bloco no canvas para editar suas propriedades.</p>
            </div>
          )}
          <div className="mx-4 mb-4 mt-5 border-t border-[var(--admin-line)] pt-4 sm:mx-5 sm:mb-5">
            <div className="flex flex-wrap gap-2">
              <button type="button" className="admin-button is-ghost" onClick={() => moveBlock(selectedBlock?.id || '', -1)} disabled={!selectedBlock}>
              ...
              <button
                type="button"
                className="admin-button is-ghost text-rose-300"
                onClick={() => selectedBlock && void removeBlock(selectedBlock)}
                ...
                <article key={snapshot.id} className="flex flex-col gap-3 px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-5">
                  ...
                  <div className="flex flex-wrap gap-2 sm:shrink-0">
                    ...
                    <button
                      ...
                      disabled={saving || snapshotSaving}
                      aria-label={`Restaurar ${snapshot.label} no rascunho`}
                    >
                      <RotateCcw size={14} />
                      Restaurar
                    </button>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border border-[var(--admin-line)] bg-[var(--admin-surface)] px-4 py-3 text-xs text-[var(--admin-dim)]">
        <span>
          <Check size={14} className="mr-1 inline text-emerald-400" aria-hidden="true" />
          Última publicação: {new Date(state.publishedAt).toLocaleString('pt-BR')}
        </span>
        <button type="button" className="admin-button is-ghost" onClick={() => void handleDiscardServerDraft()} disabled={saving || dirty}>
          Restaurar última publicação
        </button>
      </div>
      <AdminSaveBar
        dirty={dirty}
        ...
        label="Há alterações locais no rascunho da Home"
      />

      <AdminDrawer
        open={Boolean(selectedSnapshot)}
        title={selectedSnapshot?.label || 'Snapshot da Home'}
        description={selectedSnapshot ? `${snapshotKindLabel(selectedSnapshot.kind)} · versão ${selectedSnapshot.version}` : undefined}
        ...
        {selectedSnapshot && (
          <div className="space-y-5">
            <div className="grid gap-3 sm:grid-cols-3">
              <div className="border border-[var(--admin-line)] bg-[var(--admin-panel-raised)] p-3">
                <p className="admin-eyebrow">Registrado em</p>
                <p className="mt-1 text-sm font-semibold text-[var(--admin-text)]">{formatSnapshotDate(selectedSnapshot.createdAt)}</p>
              </div>
              <div className="border border-[var(--admin-line)] bg-[var(--admin-panel-raised)] p-3">
                <p className="admin-eyebrow">Blocos visíveis</p>
                <p className="mt-1 font-mono-data text-lg font-bold text-[var(--admin-text)]">{selectedSnapshot.visibleBlockCount}</p>
              </div>
              <div className="border border-[var(--admin-line)] bg-[var(--admin-panel-raised)] p-3">
                <p className="admin-eyebrow">Responsável</p>
                <p className="mt-1 truncate text-sm font-semibold text-[var(--admin-text)]">{selectedSnapshot.createdBy || 'Administrador'}</p>
              </div>
            </div>
            <section className="border border-[var(--admin-line)]">
              <div className="border-b border-[var(--admin-line)] px-4 py-3">
                <p className="admin-eyebrow">Composição preservada</p>
                <p className="mt-1 text-xs text-[var(--admin-muted)]">A ordem e as configurações abaixo serão copiadas para o rascunho ao restaurar.</p>
              </div>
              <ol className="divide-y divide-[var(--admin-line)]" aria-label="Blocos do snapshot">
                {[...selectedSnapshot.document.blocks]
                  .sort((a, b) => a.order - b.order)
                  .map((block) => (
                    <li key={block.id} className="flex items-center gap-3 px-4 py-3">
                      <span className="w-7 font-mono text-xs text-[var(--admin-dim)]">{String(block.order).padStart(2, '0')}</span>
                      <span className="min-w-0 flex-1">
                        <strong className="block truncate text-sm text-[var(--admin-text)]">{blockLabel(block)}</strong>
                        <small className="block truncate text-xs text-[var(--admin-muted)]">
                          {block.type} · {block.enabled ? 'Visível' : 'Oculto'}
                        </small>
                      </span>
                    </li>
                  ))}
            ...
            <div className="flex flex-wrap items-center justify-end gap-2 border-t border-[var(--admin-line)] pt-4">
              {selectedSnapshot.kind === 'PUBLISHED' && selectedSnapshot.version === state.publishedVersion ? (
                <span className="admin-status-badge is-unknown">
                  <LockKeyhole size={13} aria-hidden="true" /> Publicação atual protegida
                </span>
              ) : (
              ...
              <button type="button" className="admin-button is-primary" onClick={() => void restoreSnapshot(selectedSnapshot)} disabled={snapshotSaving}>
                <RotateCcw size={15} /> Restaurar no rascunho
              </button>
            </div>
          </div>
        )}
