...
import { ListFilter, Search, Sparkles, ChevronLeft, ChevronRight, X, Type, Filter } from 'lucide-react';
...
export default function AnimeListPage() {
  ...
  const configuredOrder = config.catalog.defaultSort === 'year' ? 'start_date' : config.catalog.defaultSort;
  ...
  const [quickFilters, setQuickFilters] = useState<QuickFilterState>(() => ({ ...DEFAULT_QUICK_FILTERS, orderBy: configuredOrder }));
  ...
  const {
    ...
    queryKey: ['animeListAlphabetical', selectedLetter, searchQuery, page, typeFilter, quickFilters, config.catalog.defaultPageSize],
    queryFn: () =>
      kenjitsuService.searchAnime(searchQuery, page, config.catalog.defaultPageSize, {
        ...
        letter: selectedLetter !== 'Todos' ? (selectedLetter === '#' ? '#' : selectedLetter.toLowerCase()) : undefined,
  ...
  const visibleAnime = applyCatalogPresentation(animeListData?.data, config.catalog);
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 py-8 space-y-8 animate-fade-in">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-[#0F172A] via-[#131E3A] to-[#0D111E] border border-blue-500/20 p-6 sm:p-10 shadow-2xl">
        ...
        <div className="relative z-10 space-y-4 max-w-2xl">
          ...
          <h1 className="text-2xl sm:text-4xl font-black text-white tracking-tight leading-tight">{config.catalog.pageHeadings.catalog}</h1>
      ...
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
          ...
          <div className="flex items-center gap-2 bg-white/5 border border-white/10 p-1.5 rounded-2xl">
            ...
            >
              ...
              <option value="tv" className="bg-[#12131C]">
                Séries de TV
              </option>
              <option value="movie" className="bg-[#12131C]">
                Filmes
              </option>
              <option value="ova" className="bg-[#12131C]">
                OVAs
              </option>
              <option value="ona" className="bg-[#12131C]">
                ONAs / Web
              </option>
            </select>
          </div>
        ...
        >
          ...
          {(quickFilters.genre !== 'all' ||
            quickFilters.status !== 'all' ||
            quickFilters.orderBy !== 'popularity' ||
            typeFilter !== 'all' ||
            selectedLetter !== 'Todos') && (
            <span className="grid min-w-5 place-items-center rounded-full bg-[var(--accent)] px-1.5 py-0.5 text-[10px] text-black">Ativos</span>
          )}
        </button>

        {showAdvancedFilters && (
          <div id="catalog-advanced-filters" className="space-y-4" aria-label="Filtros avançados">
            <QuickMultiFilter filters={quickFilters} onChange={handleQuickFilterChange} onReset={handleResetQuickFilters} />
