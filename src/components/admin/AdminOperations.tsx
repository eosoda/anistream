...
export function SystemOperation() {
  const [enabled, setEnabled] = useState(false); const [messageText, setMessageText] = useState('Estamos em manutenção para atualização de servidores.'); const [status, setStatus] = useState('');
  ...
  return <OperationForm title="Modo manutenção" onSubmit={submit} message={status}><FormField label="Mensagem pública">{(props) => <textarea {...props} rows={4} value={messageText} onChange={(e) => setMessageText(e.target.value)} className="admin-field" />}</FormField><Button type="submit" variant={enabled ? 'danger' : 'primary'}>{enabled ? 'Desativar manutenção' : 'Ativar manutenção'}</Button></OperationForm>;
}

export function BackupOperation() {
  const [message, setMessage] = useState('');
  const restore = async (file?: File) => { if (!file) return; try { const data = JSON.parse(await file.text()); const payload = await post('/api/admin/backup', data); setMessage(payload.message || 'Backup restaurado.'); } catch (error) { setMessage(error instanceof Error ? error.message : 'Arquivo inválido.'); } };
  return <section className="operation-card"><h2 className="text-lg font-bold">Backup e restauração</h2><p className="text-sm text-[var(--text-secondary)]">Exporte uma cópia antes de restaurar dados. A restauração altera o catálogo atual.</p><div className="flex flex-wrap gap-3"><a href="/api/admin/backup" download className="inline-flex min-h-11 items-center rounded-[var(--radius-control)] bg-[var(--accent)] px-4 text-sm font-semibold text-white">Exportar JSON</a><label className="inline-flex min-h-11 cursor-pointer items-center rounded-[var(--radius-control)] border border-[var(--border-strong)] px-4 text-sm font-semibold">Restaurar JSON<input type="file" accept="application/json,.json" className="sr-only" onChange={(e) => restore(e.target.files?.[0])} /></label></div><StatusRegion>{message}</StatusRegion></section>;
}
