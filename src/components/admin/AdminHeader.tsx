...
export function AdminHeader({ onOpenMobileSidebar, onOpenCommandPalette }: { onOpenMobileSidebar: () => void; onOpenCommandPalette: () => void }) {
  ...
  return (
    <header className="admin-header sticky top-0 z-30 flex items-center justify-between gap-4 px-4 sm:px-6">
      <div className="flex min-w-0 items-center gap-3">
        <button type="button" onClick={onOpenMobileSidebar} className="admin-header-button lg:hidden" aria-label="Abrir menu lateral">
          <Menu size={19} />
        </button>
        <nav aria-label="Breadcrumb" className="hidden min-w-0 items-center gap-1 text-xs text-[var(--admin-dim)] sm:flex">
          <span className="inline-flex items-center gap-1 text-[var(--admin-muted)]">
            <ShieldCheck size={14} className="text-[var(--success)]" /> Operação
          </span>
          {segments.map((segment, index) => (
            <span
              key={`${segment}-${index}`}
              className={cn('inline-flex min-w-0 items-center gap-1', index === segments.length - 1 && 'text-[var(--admin-text)]')}
            >
      ...
      <div className="flex shrink-0 items-center gap-1.5 sm:gap-2">
        ...
        <div className="hidden items-center gap-2 border-l border-[var(--admin-line)] pl-2 sm:flex">
          <span className="grid size-8 place-items-center rounded-[9px] bg-[var(--admin-panel-raised)] text-[var(--accent)]">
            <User size={15} />
          </span>
          <span className="hidden text-left lg:block">
            <strong className="block text-xs text-[var(--admin-text)]">Administrador</strong>
            <small className="block text-[.68rem] text-[var(--success)]">Sessão ativa</small>
          </span>
        </div>
        <button
          type="button"
