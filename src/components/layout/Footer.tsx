...
export function Footer() {
  ...
  if (!config.branding.showFooter) return null;
    ...
    <footer className="mt-10 w-full border-t border-white/10 bg-[#07070A] text-gray-400 md:mt-16">
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 md:py-10">
        <div className="mb-8 grid grid-cols-2 gap-x-6 gap-y-8 md:grid-cols-4 md:gap-8">
          <div className="col-span-2 space-y-3 md:space-y-4">
            <Link href="/" className="flex items-center gap-2">
              <div className="relative flex h-8 w-8 items-center justify-center overflow-hidden rounded-lg bg-[var(--accent)]">
                <SafeImage
              ...
              <span className="text-xl font-black tracking-wider text-white">{config.branding.brandText}</span>
            </Link>
            <p className="max-w-md text-xs leading-5 text-gray-400">{config.communication.footerDescription}</p>
            <div className="flex items-center gap-1 text-xs text-gray-400 md:pt-1">
              {config.communication.footerCredit || (
                <>
                  Desenvolvido com <Heart size={12} className="fill-current text-[#FF6B00]" /> para a comunidade otaku.
                </>
              )}
            </div>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2 text-xs font-semibold">
              {items.map((item) => (
                <li key={item.id}>
                  <Link href={item.href} className="transition-colors hover:text-[#FF6B00]">
                    {item.label}
                  </Link>
                </li>
              ))}
              <li>
                <Link href="/lista/importar" className="transition-colors hover:text-[#FF6B00]">
                  Importar Lista
                </Link>
              </li>
            </ul>
          ...
          <div>
            ...
            <ul className="space-y-2 text-xs font-semibold">
              {config.features.changelog && (
                <li>
                  <Link href="/changelog" className="transition-colors hover:text-[#FF6B00]">
                    Changelog
                  </Link>
                </li>
              )}
        ...
        <div className="flex flex-col items-start justify-between gap-2 border-t border-white/5 pt-5 text-xs text-gray-400 sm:flex-row sm:items-center">
          ...
          {config.communication.showVersionBadge && (
            <Link
              href="/changelog"
              aria-label={`Ver changelog ${config.communication.versionLabel}`}
            ...
            >
              {config.communication.versionLabel}
            </Link>
          )}
        </div>
      </div>
